...
### Rule 4: No duplicate titles

A game cannot be saved if another game with the same title already exists
in the inventory. Titles are compared case-insensitively — "hollow knight"
and "Hollow Knight" are considered the same title.

- **Where it's enforced:** `useGames.ts` — checked before saving
- **Where it's reflected:** `GameForm.tsx` — error message displayed if
  duplicate is detected
- **Why title only, not title + platform:** This app tracks a single user's
  personal game library. The same game on multiple platforms is an edge case
  that adds complexity without enough real-world value to justify it.

### Rule 5: Title has a character limit of 100

A game title cannot exceed 100 characters. This is long enough to accommodate
any real game title while keeping the UI clean.

- **Where it's enforced:** `validateGame` in `gameUtils.ts` and `GameForm.tsx`

### Rule 6: Title is trimmed before saving

Leading and trailing whitespace is stripped from the title before saving.
`"  Hollow Knight  "` becomes `"Hollow Knight"`. A title that is only
whitespace is treated as empty and fails validation.

- **Where it's enforced:** `validateGame` in `gameUtils.ts`

### Rule 7: Rating is required

A game cannot be saved without a rating. Rating must be between 1 and 5.
A value of 0 means unrated — the form will not allow saving until a star
is selected.

- **Where it's enforced:** `validateGame` in `gameUtils.ts` and `GameForm.tsx`

### Rule 8: Progress can be set by typing a number

The progress slider is the primary input, but users can also type a number
directly. The value must be between 0 and 100.

- **Where it's enforced:** `validateGame` in `gameUtils.ts` and `GameForm.tsx`

### Rule 9: Add form always opens blank

Opening the Add form always shows a blank form regardless of any previous
state. No values are pre-filled from a previous session.

- **Where it's enforced:** `App.tsx` — modal state resets on open

### Rule 10: Title is required
